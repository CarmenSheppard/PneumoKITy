...
def translate_mixmm(mix_mm, session):
    """
    Translate mixmm dictionary into percentages and return as list
    """
    new_mixmm=[]
    ...
    if len(set(groups)) > 1 or groups and types:
        ...
        if len(pheno) > 1:
            ...
            if max(df["median-multiplicity"]) > 1:
               ...
               mix_mm = translate_mixmm(mix_mm, session)
    ...
    try:
        ...
        if os.path.isfile(tsvfile) and os.path.getsize(tsvfile) > 0:
            ...
            if not filtered_df.empty:
                ...
                if analysis.category == Category.mix and analysis.runtype == "pure":
                    analysis.rag_status = "AMBER"

                elif analysis.category == Category.mix and analysis.runtype == "mix":
